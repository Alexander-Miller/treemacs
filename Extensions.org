...
the builtin ~button.el~ library. Its functions can all be invoked on treemacs nodes, including ~button-get~, which we
use here to retrieve the list of buffers that we will have stored in the node's ~:buffers~ property.
...
first we need another, final building block. We are building an extension for projects, we have yet to decide /which/
projects it is for. In other words we need a predicate. So let's assume we want our extension to show up only for the
...
the extension will rendered at the very start or the very end of the project's immediate children.

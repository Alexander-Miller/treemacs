...
An action to open the buffer on RET:
...
Bottom-up definition. First the leaves - nodes for single buffers:
...
Put it at the top of the treemacs buffer:

#+BEGIN_SRC emacs-lisp
  (treemacs-enable-top-level-extension
   :extension treemacs-showcase-buffers-extension-instance
   :position 'top)
#+END_SRC

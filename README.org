...
workspace contains /exactly/ a single project you can use ~M-H~ and ~M-L~ (or ~treemacs-root-up~ and ~treemacs-root-down~)
to arbitrarily change the single project's root. ~M-H~ will navigate one level upward in the file system, ~M-L~ will move

...
- Is it possible to tweak the imenu tree nodes, such that more details are shown?

   Previously treemacs supported the `treemacs-elisp-imenu-expression` customization option,
   which improves the imenu view for elisp files. This option has been removed in order to avoid inference
   with user settings.

   In order to improve the imenu looks, you might want to configure the variable `imenu-generic-expression`.
   For example the following configuration can be put inside an `emacs-lisp-mode-hook`.

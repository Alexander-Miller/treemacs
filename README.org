 ...
 * Calling ~treemacs-push-button~ on a tag will make the jump to the tag's definition, but there is now way to decide
   which window will be used, like with the [[#default-keymap][treemacs-visi-file-*]] functions. For now treemacs will
   simply make the call using ~next-window~.

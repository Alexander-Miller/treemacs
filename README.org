 ...
 * Bookmark integration :: Running ~bookmark-set~ on a Treemacs item will store a bookmark to Treemacs buffer for that item.
   Bookmarking all kinds of items except for tags is supported.
...
default theme; this feature is meant to easily allow *others* to create and distribute themes for treemacs.
...
file extensions the icon should be used for (for treemacs an extension is either the text after the last period or the
entire file name).

...
(defface treemacs-mu4e-mailcount-face
  ...
  "Face for message count information when all messages are read."
  :group 'treemacs-mu4e)

(defface treemacs-mu4e-mailcount-unread-face
  '((t :inherit font-lock-doc-face))
  "Face for message count information when there are unread messages."
  :group 'treemacs-mu4e)
...
(defun treemacs-mu4e--display ()
  ...
  (let* ((buf (get-buffer-create treemacs-mu4e--buffer-name))
    ...
    (treemacs-initialize 'mu4e-top-maildirs
      (setq-local treemacs-space-between-root-nodes t))
    (treemacs-mu4e--update-mailcounts)
    (treemacs--evade-image)))

;;;;; Async Mailcount

;;; TODO(2021/08/15): pfuture buffer nicht getötet bei fehler
(defun treemacs-mu4e--update-mailcounts ()
  ...
  (-let [maildirs (mapcar #'treemacs-maildir->true-folder
    ...
    (pfuture-callback `("python"
                        ...
                        ,@maildirs)
      :on-error (treemacs-log "Mail count update error: %s" (pfuture-callback-output))
      :on-success
      (-let [source "treemacs-mu4e-mailcount"]
        (treemacs-clear-annotation-suffixes source)
        (pcase-dolist (`(,path ,suffix ,face) (read (pfuture-callback-output)))
          (put-text-property 0 (length suffix) 'face face suffix)

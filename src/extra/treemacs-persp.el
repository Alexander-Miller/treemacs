...
(cl-defmethod treemacs-scope->cleanup ((_ (subclass treemacs-persp-scope)))
  ...
  (remove-hook 'persp-renamed-functions #'tremacs-persp--on-perspective-rename)
  (remove-hook 'persp-before-kill-functions #'treemacs--on-scope-kill))

(defun treemacs-persp--on-perspective-rename (perspective old-name new-name)
  "Hook running after perspective was renamed.
Will rename treemacs perspective workspace to use new name."
  (treemacs-rename
   (treemacs--find-workspace-by-name (treemacs-perspective--format-workspace-name old-name))
   (treemacs-perspective--format-workspace-name new-name)))
